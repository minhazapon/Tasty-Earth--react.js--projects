



const Footer = () => {
    return (
        <div className=" flex-col md:flex-row lg:flex-row border-[2px] border-black mt-24">

            <div className=" flex-col md:flex-row lg:flex-row">
                
              
             <div className=" flex-col md:flex-row lg:flex-row  bg-no-repeat  bg-cover p-52 " 
              style={{backgroundImage: 'url(https://img.freepik.com/premium-photo/collaboration-chefs-restaurant-generative-ai_1169649-10784.jpg?w=740)'}}
             >

             <div className=" bg-gradient-to-r from-black p-5 text-center">
                   <h1 className=" text-white text-xl">Wants to hook better leads with hard-<br></br>Working Food Marketing</h1>
                   <button className=" btn  mt-5 w-[150px] bg-yellow-500 text-white">Get Call Us</button>
             </div>
             </div>

            
             </div>

{/* -------------------------------------------------------- */}

             <div>
                
                
             <footer className="footer p-10 bg-blue-950 text-white ">
  <div>

    <div className=" flex items-center gap-3">
        <img className=" h-[40px]" src="https://i.ibb.co/L52RsRD/bag.png" alt="" />
        <h1 className=" text-xl font-bold text-yellow-500 ">Tasty Earth</h1>
        
    </div> 
     
     <h1 className=" text-xl font-bold">New London, street 5001, England</h1>
     <h1 className=" text-xl font-bold">"People who love to eat are always the best people."</h1>

    
    
  </div>  
  <div className=" flex-col md:flex-row lg:flex-row">
  <p className="  bg-white h-[200px] w-[1px] text-center "></p>
  </div>

  <nav>
    <h6 className=" text-xl font-bold text-yellow-500 ">Company</h6> 
    <a className="link link-hover">About us</a>
    <a className="link link-hover">Contact</a>
    <div className=' flex items-center gap-2'>
     <img className=' h-[30px]' src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/cd/Facebook_logo_%28square%29.png/800px-Facebook_logo_%28square%29.png" alt="" /> 
    <a className="link link-hover">Facebook</a>
    </div>
    <div className=' flex items-center gap-2'>
      <img className=' h-[30px]' src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/LinkedIn_logo_initials.png/600px-LinkedIn_logo_initials.png?20140125013055" alt="" />
    <a className="link link-hover">Linkedin</a>
    </div>
    <div className=' flex items-center gap-2'>
      <img className=' h-[30px]' src="https://seeklogo.com/images/T/twitter-2012-negative-logo-5C6C1F1521-seeklogo.com.png" alt="" />
    <a className="link link-hover">Twitter</a>
    </div>
    
  </nav> 

  <p className=" flex-col md:flex-row lg:flex-row bg-white h-[200px] w-[1px] text-center "></p>

 
  <form>
    <h6 className="footer-title">Wants Our Best Marketing Tips?</h6> 
    <fieldset className="form-control w-80">
      <label className="label">
        <span className="  text-black ">Enter your email address</span>
      </label> 
      <div className="join">
        <input type="text" placeholder="username@site.com" className="input text-black input-bordered join-item" /> 
        <button className="btn btn-primary join-item">Subscribe</button>
      </div>
    </fieldset>
  </form>


</footer>    
                
                
                
                
                
<footer className="footer items-center p-4  bg-blue-950 text-neutral-content">
  <aside className="items-center grid-flow-col">
    <svg width="36" height="36" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd" className="fill-current"><path d="M22.672 15.226l-2.432.811.841 2.515c.33 1.019-.209 2.127-1.23 2.456-1.15.325-2.148-.321-2.463-1.226l-.84-2.518-5.013 1.677.84 2.517c.391 1.203-.434 2.542-1.831 2.542-.88 0-1.601-.564-1.86-1.314l-.842-2.516-2.431.809c-1.135.328-2.145-.317-2.463-1.229-.329-1.018.211-2.127 1.231-2.456l2.432-.809-1.621-4.823-2.432.808c-1.355.384-2.558-.59-2.558-1.839 0-.817.509-1.582 1.327-1.846l2.433-.809-.842-2.515c-.33-1.02.211-2.129 1.232-2.458 1.02-.329 2.13.209 2.461 1.229l.842 2.515 5.011-1.677-.839-2.517c-.403-1.238.484-2.553 1.843-2.553.819 0 1.585.509 1.85 1.326l.841 2.517 2.431-.81c1.02-.33 2.131.211 2.461 1.229.332 1.018-.21 2.126-1.23 2.456l-2.433.809 1.622 4.823 2.433-.809c1.242-.401 2.557.484 2.557 1.838 0 .819-.51 1.583-1.328 1.847m-8.992-6.428l-5.01 1.675 1.619 4.828 5.011-1.674-1.62-4.829z"></path></svg> 
    <p>Copyright © 2024 - All right reserved By Minhazul Abedin Apon </p>
  </aside> 
  <nav className="grid-flow-col gap-4 md:place-self-center md:justify-self-end">
    <a href="https://www.linkedin.com/in/minhazul-abedin-apon-346b52297/">

        <img className="  h-[30px]" src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/LinkedIn_logo_initials.png/600px-LinkedIn_logo_initials.png?20140125013055" alt="" />
        
    </a>

    
    <a href="https://www.facebook.com/profile.php?id=100011336918123" ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"></path></svg></a>
  </nav>
</footer>   
                
     
                
                
                
                
                
                
                
                
                
                
                
                
                
                
                
                
            </div>  










            
        </div>
    );
};

export default Footer;